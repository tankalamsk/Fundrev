  ...
  const handleUpload = async () => {
    try {
      ...
      reader.onload = async (e) => {
        ...
        // Send jsonArray to the backend
        try {
          const response = await fetch('http://localhost:4000/assignment/src/pages/StartupDashboard', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify({
              email: props.email, // Include the necessary data
